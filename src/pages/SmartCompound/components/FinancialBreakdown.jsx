...
import { fmt, SERVICES_CODES } from "../data";
...
export default function FinancialBreakdown({ lines, info }) {
  return (
    <section className="mb-12">
      ...
      >
          ...
          <table className="w-full text-sm min-w-[600px]">
            ...
            <tbody className="divide-y divide-white/5">
              {lines.map((line, i) => {
                ...
                const isAddon = SERVICES_CODES.includes(line.code);
                return (
                  <motion.tr
                    key={line.code}
                    initial={{ opacity: 0 }}
                    ...
                    className={`text-gray-300 hover:bg-white/5 transition-colors ${isAddon ? "bg-brand-green/5" : ""}`}
                  >
        ...
        <div className="border-t border-white/10 px-6 py-5 space-y-2.5">
          <Row label="المجموع قبل نسبة الطوارئ" value={info.subtotalEGP} />
          <Row
            label={`نسبة الطوارئ (${fmt(info.contingencyPercent)}%)`}
            value={info.contingencyEGP}
            muted
          />
          <div className="border-t border-brand-green/20 pt-3">
            <Row label="الإجمالي النهائي" value={info.totalEGP} highlight />
          </div>
        </div>
      </motion.div>

      <p className="text-gray-500 text-xs mt-3 leading-relaxed">
        نسبة الطوارئ {fmt(info.contingencyPercent)}% مخصصة لتغطية التعديلات الطبيعية أثناء التنفيذ ودورات المراجعة
        الإضافية، ولا تُصرف إلا عند الحاجة الفعلية إليها.
      </p>
    </section>
  );
}
